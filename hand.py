...
class Controller:
    ...
    def handle_controls(gesture, hand_result):  
        ...
        elif gesture == (Gest.INDEX | Gest.PINKY):
            # Start timer when the gesture is detected
            start_time = time.time()

            # Keep checking if the gesture is maintained for 3 seconds
            while time.time() - start_time < 3:
                # Check if the gesture is still maintained
                if gesture != (Gest.INDEX | Gest.PINKY):
                    # If the gesture changes before 3 seconds, break the loop
                    break
                time.sleep(0.1)  # Adjust sleep time as needed for better performance
            
            # If the gesture is maintained for 3 seconds, open YouTube in a web browser
            if time.time() - start_time >= 3:
                webbrowser.open("https://www.youtube.com", new=1)
                webbrowser.open("https://www.google.com", new=2)
                webbrowser.open("https://web.whatsapp.com", new=3)
                
            if gesture == (Gest.THUMB | Gest.INDEX):
            # Simulate zooming action
               pyautogui.scroll(30)
